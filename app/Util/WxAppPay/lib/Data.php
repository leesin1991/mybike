...
class WxPayDataBase
{
	...
	**/
	 ...
	 * @param string $value
	 **/
	public function SetAppSign($data=[])
	{
		$sign = $this->MakeAppSign($data);
		$this->values['sign'] = $sign;
		return $sign;
	}
	...
	**/
	...
	/**
	 * 格式化参数格式化成url参数,自定义的app使用
	 */
	public function ToUrlAppParams($data=[])
	{
		$buff = "";
		foreach ($data as $k => $v)
		{
	 ...
	 * @return 签名，本函数不覆盖sign成员变量，如要设置签名需要调用SetSign方法赋值
	 */
	public function MakeAppSign($data=[])
	{
		//签名步骤一：按字典序排序参数
		ksort($data);
		$string = $this->ToUrlAppParams($data);
		//签名步骤二：在string后加入KEY
		$string = $string . "&key=".WxPayConfig::KEY;
		//签名步骤三：MD5加密
		$string = md5($string);
 ...
 * @param string $value
 **/
	public function setPackage_app($value)
	{
		$this->values['package_app'] = $value;
	}
	/**
	* 获取订单详情扩展字符串的值
	* @return 值
